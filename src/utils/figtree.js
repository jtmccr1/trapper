...
export default class Tree {
	...
	constructor(rootNode = {}) {
		...
		this.nodeList.forEach((node, index) => (node.key = `node ${index}`));
	...
	*postorder(startNode = this.root) {
	...
	toNewick(node = this.rootNode) {
		return (
			(node.children
				? `(${node.children.map(child => this.toNewick(child)).join(',')})${node.label ? node.label : ''}`
	...
	update() {
		this.nodeList = [...this.preorder()];
		this.nodeList.forEach((node, index) => (node.key = Symbol.for(`node ${index}`)));
		this.nodeMap = new Map(this.nodeList.map(node => [node.key, node]));
	}
	/**
