        ...
        if (node !== null) {
            if(node.children.length===0){ // make it the first time
                ...
                transitionDuration:300};
                    const margins = {...props.margins,...{top:32,bottom:23}};
...
const rand_id = `b${Math.random().toString(36).substring(4)}`
        ...
        return(
            <svg id= {rand_id} 
            ref={el}
            height={props.domRect.height+60}// 10px padding timecontainer 10px margin on chart 20px top margin on this 20 px bottom here
